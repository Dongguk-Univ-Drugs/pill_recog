...
def get_color(src, low, high, color, color_arr):
    ...
    for cnt in contours:
        ...
        if area > 20000 and area < 40000:
            ...
            cv2.imwrite('grabcut/src_hsv.png', src_hsv)

def get_circle(src, file):
    gray_image = cv2.cvtColor(src, cv2.COLOR_BGR2GRAY)
    # circles = cv2.HoughCircles(gray_image, cv2.HOUGH_GRADIENT, 1, 100,
    #                            param1=180, param2=30, minRadius=0, maxRadius=0)  # 수치 변경 가능
    # circles = cv2.HoughCircles(gray_image, cv2.HOUGH_GRADIENT, 1, gray_image.shape[1]/3,
    #                            param1=150, param2=30, minRadius=0, maxRadius=0)  # 수치 변경 가능
    circles = cv2.HoughCircles(gray_image,cv2.HOUGH_GRADIENT,1,100,
    ...
    if circles is not None:
        ...
        cv2.imwrite(f'grabcut/{file}_gray.png', gray_image)
        boundR = cv2.boundingRect(gray_image)
        print(f"{boundR} vs {gray_image.shape}")
        cv2.imwrite(f'grabcut/{file}_circle.png', gray_image[boundR[0]:boundR[2], boundR[1]:boundR[3]])

        # 원 감지됐을 때
        return get_line(src, file)    # return: 0 --> 원, 1 --> 원 아님

def get_line(src, file):
    ...
    if lines is not None:
        ...
        cv2.imwrite(f'grabcut/{file}_src.png', src)
        return 1
...
if __name__ == "__main__":
    ...
    for file, file_type in files:
        if 'image' in file:
            ...
            src = cv2.imread(f'{test_image_path}/{file}.{file_type}')
            if src is None:
            ...
            # 모양
            circle = get_circle(src, file)

            # 색상, 모양에 따라 분류
            path = r"../pill_db.xlsx"
            wb = None#op.load_workbook(path)
            ws = None#wb.active
            ...
            cv2.imwrite(f'grabcut/{file}.png', foreground)
            ...
            ret, img_threshold = cv2.threshold(
                clahe_foreground, 100, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)

            closing_image = closing(img_threshold)
            cv2.imwrite(f'grabcut/{file}_closing_image.png', closing_image)
            # boundR = cv2.boundingRect(closing_image)
            # print(f"width : {image.shape[1]} and height : {image.shape[0]}")
            # print(f"boundR : {boundR}")
            # img = image[boundR[0]:boundR[2],boundR[1]:boundR[3]]
            # cv2.imwrite(f'grabcut/{file}_result.png', img)
            # ----------------------------------------
            ...
            cv2.imwrite(f'grabcut/{file}_thresh.png', thresh)

            #cv2.imshow("canny", canny)
            cv2.imwrite(f'grabcut/{file}_canny.png', canny)
